...
function createObjectStores(db) {
  ...
  usersStore.createIndex('signups', 'signups', {
    unique: false,
    multiEntry: true,
  }); // Array of { listing_id, status, created_ts }
  ...
  listingsStore.createIndex('last_modified_ts', 'last_modified_ts', {
    unique: false,
  });
  ...
  listingsStore.createIndex('sign_ups_limit', 'sign_ups_limit', {
    unique: false,
  });
  listingsStore.createIndex('vc_properties', 'vc_properties', {
    unique: false,
  });
  listingsStore.createIndex('signups', 'signups', {
    unique: false,
    multiEntry: true,
  }); //array of [user_id, status]
  listingsStore.createIndex('tags', 'tags', {
    unique: false,
    multiEntry: true,
  }); //array of tag_id

  // User-Listings junction store
  const userListingsStore = db.createObjectStore('user_listings', {
    keyPath: ['user_id', 'listing_id'],
  });
  ...
  tagsStore.createIndex('last_modified_ts', 'last_modified_ts', {
    unique: false,
  });
  ...
  // VC Issue Jobs store
  const vcIssueJobsStore = db.createObjectStore('vc_issue_jobs', {
    keyPath: 'id',
  });
  vcIssueJobsStore.createIndex('status', 'status', { unique: false });
  vcIssueJobsStore.createIndex('created_ts', 'created_ts', { unique: false });
  vcIssueJobsStore.createIndex('last_modified_ts', 'last_modified_ts', {
    unique: false,
  });
  vcIssueJobsStore.createIndex('vc_properties', 'vc_properties', {
    unique: false,
  });

  // Admin configs store
  const admin_configsStore = db.createObjectStore('admin_configs', {
    keyPath: 'id',
  });
  admin_configsStore.createIndex('admin_ocids', 'admin_ocids', {
    unique: false,
  });
}
