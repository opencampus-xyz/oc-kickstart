...
export class DBService {
  ...
  async queryPendingVCJobs() {
    if (!this.db) return [];
    const transaction = this.IndexedDBHelper.createTransaction(
      ["vc_issue_jobs"],
      "readonly"
    );
    const store = this.IndexedDBHelper.getStore(transaction, "vc_issue_jobs");
    const statusIndex = this.IndexedDBHelper.getIndex(store, "status");

    const pendingJobs = await this.IndexedDBHelper.getAll(
      statusIndex,
      VcIssueJobStatus.PENDING
    );
  ...
  async createUser(userData) {
    ...
    const existingUser = await this.IndexedDBHelper.getByIndex(
      userIndex,
      userData.oc_id
    );
  ...
  async getUserByOCId(ocId) {
    ...
    const tx = this.IndexedDBHelper.createTransaction(
      ["users", "admin_configs"],
      "readonly"
    );
    ...
    const adminConfig = await this.IndexedDBHelper.get(
      adminStore,
      "admin_config"
    );
    ...
    const isAdmin =
      isMasterAdmin || adminConfig?.admin_ocids?.includes(ocId) || false;
  ...
  async getUserSignups({ page = 0, pageSize = 10, searchText, userId }) {
    ...
    const tx = this.IndexedDBHelper.createTransaction(
      ["user_listings", "listings", "vc_issue_jobs"],
      "readonly"
    );
    const userListingsStore = this.IndexedDBHelper.getStore(
      tx,
      "user_listings"
    );
    ...
    const userListingsIndex = this.IndexedDBHelper.getIndex(
      userListingsStore,
      "user_id"
    );
        ...
        if (cursor) {
          ...
          if (signup.user_id === userId) {
            ...
            listingRequest.onsuccess = () => {
              ...
              if (!listing) {
                console.warn(
                  `Listing not found for signup: ${signup.listing_id}`
                );
                cursor.continue();
                return;
              }
              ...
              vcJobsRequest.onsuccess = (vcEvent) => {
                ...
                if (vcCursor) {
                  ...
                  if (vcCount > 0) {
                    const vcPendingCount = vcJobs.filter(
                      (job) => job.status === VcIssueJobStatus.PENDING
                    ).length;
                    const vcFailedCount = vcJobs.filter(
                      (job) => job.status === VcIssueJobStatus.FAILED
                    ).length;
                    const vcSuccessCount = vcJobs.filter(
                      (job) => job.status === VcIssueJobStatus.SUCCESS
                    ).length;
  ...
  async signupForListing(userId, listingId) {
    ...
    const existingSignup = await this.IndexedDBHelper.get(userListingsStore, [
      userId,
      listingId,
    ]);
    ...
    const signupCount = listing.signups.filter(
      ([_, status]) => status === "pending" || status === "approved"
    ).length;
  ...
  async getUserListingStatus(userId, listingId) {
    const tx = this.IndexedDBHelper.createTransaction(
      ["user_listings"],
      "readonly"
    );
    ...
    const signup = await this.IndexedDBHelper.get(userListingsStore, [
      userId,
      listingId,
    ]);
  ...
  async createListing(listingData) {
    ...
    try {
      const listingDoc = createListingDocument({
        ...
        trigger_mode:
          listingData.triggerMode?.toLowerCase() ?? ListingTriggerMode.MANUAL,
        sign_ups_limit: parseInt(listingData.maxSignUps),
        vc_properties: {
          title: listingData.title,
          achievementType: listingData.achievementType,
          expireInDays: listingData.expireInDays
            ? parseInt(listingData.expireInDays)
            : null,
        },
      });
  ...
  async updateListingStatus(listingId, newStatus) {
    ...
    try {
      const currentListing = await this.IndexedDBHelper.get(
        listingStore,
        listingId
      );
      ...
      const missingFields = requiredFields.filter(
        (field) => !currentListing[field]
      );

      if (missingFields.length > 0) {
        console.error("Listing is missing required fields:", currentListing);
        throw new Error(
          `Listing is missing required fields: ${missingFields.join(", ")}`
        );
      }
      ...
      const updatedListing = {
        ...
        published_ts:
          newStatus === ListingStatus.ACTIVE
            ...
            : currentListing.published_ts,
        deleted_ts:
          newStatus === "deleted"
            ? new Date().toISOString()
            : currentListing.deleted_ts,
      };
  ...
  async updateSignupStatus(userId, listingId, status) {
    ...
    if (listing) {
      const listingSignupIndex = listing.signups.findIndex(
        ([uid, _]) => uid === userId
      );
      ...
      if (
        listing.trigger_mode === ListingTriggerMode.AUTO &&
        status === "completed"
      ) {
  ...
  async getTags({ page, pageSize, searchText } = {}) {
    ...
    const hasPagination = isNumber(page) && isNumber(pageSize);
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = (event) => {
        ...
        if (cursor) {
          ...
          const isWithinPageRange =
            !hasPagination ||
            (count >= page * pageSize && count < (page + 1) * pageSize);
          if (isWithinPageRange && isTagMatch) {
            if (!tag.archived_ts) {
              ...
              const formattedTag = {
                ...
                vc_properties: {
                  ...
                  achievementType:
                    tag.achievementType || vc_properties.achievementType || "",
                  expireInDays:
                    tag.expireInDays || vc_properties.expireInDays || null,
                },
              };
  ...
  async createTag(tagData) {
    ...
    const existingTag = await this.IndexedDBHelper.getByIndex(
      index,
      tagData.name
    );
    if (existingTag) {
    ...
    const tagDoc = createTagDocument({
      ...
      expireInDays: tagData.expireInDays
        ? parseInt(tagData.expireInDays)
        : null,
      vc_properties: {
        title: tagData.title || "",
        achievementType: tagData.achievementType || "",
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = async (event) => {
        ...
        if (cursor) {
          ...
          const statusMatches =
            showAllStatuses || listing.status === ListingStatus.ACTIVE;
          if (
            statusMatches &&
            this.matchesListingFilters(listing, { searchText, searchTags: searchTags?.split(',') ?? [] })
          ) {
            ...
            if (
              ...
              const userSignupStatus = await this.getUserSignupStatus(
                listing,
                userId
              );
            ...
            if (shouldIncludeListing) {
              if (
                ...
                if (includeUserSignups && userId) {
                  listing.sign_up_status = await this.getUserSignupStatus(
                    listing,
                    userId
                  );
                }
                ...
                if (listing.tags && listing.tags.length > 0) {
                  for (const tagId of listing.tags) {
                    try {
                      const tag = await this.IndexedDBHelper.get(
                        tagStore,
                        tagId
                      );
                      if (tag && !tag.archived_ts) {
    ...
    const payload = {
      ...
      credentialPayload: {
        ...
        validUntil: vcProperties.expireInDays
          ? new Date(
              now.getTime() + vcProperties.expireInDays * 24 * 60 * 60 * 1000
            )
          : undefined,
        description: listing.description,
        credentialSubject: {
  ...
  async createVCIssueJobs(userId, listingId) {
    const tx = this.IndexedDBHelper.createTransaction(
      ["vc_issue_jobs", "users", "listings", "tags"],
      "readwrite"
    );
    const vcJobsStore = this.IndexedDBHelper.getStore(tx, "vc_issue_jobs");
    const usersStore = this.IndexedDBHelper.getStore(tx, "users");
    ...
    const listingTags = allTags.filter(
      (tag) =>
        listing.tags &&
        listing.tags.includes(tag.id) &&
        tag.can_issue_oca &&
        !tag.archived_ts
    );
    ...
    const generateOCAPayload = (
      ...
      const utcValidUntil = vcProperties.expireInDays
        ? new Date(
            utcNow.getTime() + vcProperties.expireInDays * 24 * 60 * 60 * 1000
          )
        : undefined;
  ...
  matchesListingFilters(listing, { searchText, searchTags }) {
    ...
    if (searchTags && searchTags.length > 0) {
      ...
      const hasAllTags = searchTagStrings.every((searchTag) =>
        listingTagStrings.includes(searchTag)
      );
      if (!hasAllTags) {
        return false;
      }
  ...
  async adminConfig(adminOCIDs = null) {
    const tx = this.IndexedDBHelper.createTransaction(
      ["admin_configs"],
      adminOCIDs !== null ? "readwrite" : "readonly"
    );
  ...
  async removeAdmin(ocId) {
    ...
    adminConfig.admin_ocids = adminConfig.admin_ocids.filter(
      (id) => id !== ocId
    );
    adminConfig.last_modified_ts = new Date().toISOString();
    await this.IndexedDBHelper.put(store, adminConfig);
  ...
  async getUsers({ page = 0, pageSize = 10, searchText }) {
    ...
    const filteredUsers = searchText
      ? users.filter((user) =>
          user.search_text.toLowerCase().includes(searchText.toLowerCase())
        )
      : users;
  ...
  async getListingSignups(listingId) {
    const tx = this.IndexedDBHelper.createTransaction(
      ["listings", "users", "user_listings", "vc_issue_jobs"],
      "readonly"
    );
    const listingStore = this.IndexedDBHelper.getStore(tx, "listings");
    const userStore = this.IndexedDBHelper.getStore(tx, "users");
      ...
      signupsRequest.onsuccess = async (event) => {
        ...
        if (cursor) {
          ...
          if (userListing.listing_id === listingId) {
            try {
              const user = await this.IndexedDBHelper.get(
                userStore,
                userListing.user_id
              );
              ...
              if (user) {
                ...
                vcJobsRequest.onsuccess = (vcEvent) => {
                  ...
                  if (vcCursor) {
                    const job = vcCursor.value;
                    if (
                      job.user_id === userListing.user_id &&
                      job.listing_id === userListing.listing_id
                    ) {
                      vcJobs.push(job);
                    }
                    vcCursor.continue();
              ...
              console.warn(
                `Failed to fetch user ${userListing.user_id}:`,
                error
              );
              cursor.continue();
            }
